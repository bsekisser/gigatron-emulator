...
int CpuCycle(const CpuState *const newState, CpuState *const oldState)
{
...
int VgaInit(VgaState *const vga, DTime *const stats)
{
	vga->left = 0;
	vga->stats = stats;
	vga->x = 0;
...
void main_init(int argc, char* argv[], CpuState *const cpuState)
{
	srand(time(NULL));
...
{
	CpuState currentState[2];
	DTime stats;
	VgaState vga;

	main_init(argc, argv, &currentState[0]);

	int vga_init_rval = VgaInit(&vga, &stats);
	if(vga_init_rval) return(vga_init_rval);
	...
	while (!quitRequest)
	{
		int i0 = stats.t & 1, i1 = i0 ^ 1;

		CpuState *const currentState = &currentState[i0];
		CpuState *const newState = &currentState[i1];

		if (stats.t < 0) currentState->PC = 0;

		CpuCycle(currentState, newState);
